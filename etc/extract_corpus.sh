...
A (relatively) simple script for extracting GAP test files from the GAP source and packages.
...
echo "Extracting GAP $version corpus to $work_dir/${output}_gap_tst.tar.gz . . . "
...
if [ -d $work_dir/${output}_gap_tst ]; then
  rm -rf $work_dir/${output}_gap_tst
fi
mkdir -p $work_dir/${output}_gap_tst
echo "Finding and flattening all .g and .tst files . . ."
find_and_flatten "$work_dir/gap/grp $work_dir/gap/lib $work_dir/gap/tst" ".*\.\(gd\|gi\|g\|tst\)" "$work_dir/${output}_gap_tst"
echo "Running fixup script . . ."
find "$work_dir/${output}_gap_tst" -name '*.g*' -type f -exec sh -c "grep -q -E '^gap>' '{}' && mv '{}' '{}.tst'" \;
echo "Removing all .g* files . . ."
find "$work_dir/${output}_gap_tst" -name '*.g*' ! -name '*.tst' -type f -delete
echo "Compressing to $work_dir/${output}_gap_tst.tar.gz . . ."
tar -czf $work_dir/${output}_gap_tst.tar.gz -C $work_dir/${output}_gap_tst .
echo "Removing $work_dir/${output}_gap_tst . . ."
rm -rf $work_dir/${output}_gap_tst

echo "Extracting GAP $version package corpus to $work_dir/${output}_pkg_tst.tar.gz . . . "
...
if [ -d $work_dir/${output}_pkg_tst ]; then
  rm -rf $work_dir/${output}_pkg_tst
fi
mkdir -p $work_dir/${output}_pkg_tst
echo "Finding and flattening all .g and .tst files . . ."
find_and_flatten "$work_dir/pkg_$version" ".*\.\(gd\|gi\|g\|tst\)" "$work_dir/${output}_pkg_tst"
echo "Running fixup script . . ."
find "$work_dir/${output}_pkg_tst" -name '*.g*' -type f -exec sh -c "grep -q -E '^gap>' '{}' && mv '{}' '{}.tst'" \;
echo "Removing all .g* files . . ."
find "$work_dir/${output}_pkg_tst" -name '*.g*' ! -name '*.tst' -type f -delete
echo "Compressing to $work_dir/${output}_pkg_tst.tar.gz . . ."
tar -czf $work_dir/${output}_pkg_tst.tar.gz -C $work_dir/${output}_pkg_tst .
echo "Removing $work_dir/${output}_pkg_tst . . ."
rm -rf $work_dir/${output}_pkg_tst
